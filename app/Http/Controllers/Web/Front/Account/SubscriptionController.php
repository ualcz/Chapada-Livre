...
class SubscriptionController extends AccountBaseController
{
	...
	public function showForm()
	{
		...
		if (empty($user)) {
			abort(404, trans('global.user_not_found'));
		}
		...
		$title = trans('global.update_my_subscription') . ' - ' . $appName;
		
		// Meta Tags
		MetaTag::set('title', $title);
		MetaTag::set('description', trans('global.update_my_subscription'));
	...
	public function postForm(PackageRequest $request)
	{
		// Get auth user
		$authUser = auth()->user();
		abort_if(empty($authUser), 404, trans('global.user_not_found'));
		...
		abort_if(empty($user), 404, trans('global.user_not_found'));
		...
		if (!$isOfflinePayment) {
			flash(trans('global.no_payment_is_made'))->info();
		}
