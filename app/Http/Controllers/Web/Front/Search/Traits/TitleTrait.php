...
trait TitleTrait
{
	...
	public function getHtmlTitle(?array $preSearch = [], ?array $sidebar = []): string
	{
		...
		$htmlTitle .= '<span>' . trans('global.all_listings') . '</span>';
		$htmlTitle .= '</a>';
	...
	public function getBreadcrumb(?array $preSearch = []): array
	{
		...
		if (!empty($city)) {
			...
			$bcTab[] = collect([
				'name'     => trans('global.all_listings') . ' ' . $title,
		...
		if (!empty($state)) {
			...
			$bcTab[] = collect([
				'name'     => !empty($category) ? (trans('global.all_listings') . ' ' . $title) : data_get($state, 'name'),
				'url'      => $searchUrl,
				'position' => !empty($category) ? 5 : 3,
				'location' => true,
			]);
