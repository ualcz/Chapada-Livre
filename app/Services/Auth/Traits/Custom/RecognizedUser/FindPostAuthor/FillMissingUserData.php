...
trait FillMissingUserData
{
	 ...
	 */
	protected function fillMissingUserData(?User $user = null, ?object $post = null): JsonResponse
	{
		$data = [
			'success' => false,
			'message' => null,
		...
		if (!empty($post)) {
			...
			// From input (When a user contacts an author)
		...
		if (empty($user->email) && !empty($email)) {
			...
			if ($emailDoesntExist) {
				...
				$message = trans('global.email_completed');
			}
		}
		...
		if (empty($user->phone) && !empty($phone)) {
			...
			if ($phoneDoesntExist) {
				...
				$message = trans('global.phone_completed');
			}
		}
