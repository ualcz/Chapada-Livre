...
class UserSocialLoginService extends BaseService
{
	...
	public function getEntries(array $params = []): JsonResponse
	{
		...
		$message = ($socialLogins->count() <= 0) ? trans('global.no_user_social_logins_found') : null;
	...
	public function getEntry(string|int $providerOrId, array $params = []): JsonResponse
	{
		...
		abort_if(empty($socialLogin), 404, trans('global.user_social_login_not_found'));
	...
	public function destroy(string $ids): JsonResponse
	{
		$data = [
			'success' => false,
			'message' => trans('global.no_entries_deleted'),
			'result'  => null,
		];
		...
		if ($res) {
			$data['success'] = true;
			$data['message'] = trans('global.entries_deleted');
		}
