...
@section('content')
	...
	<div class="main-container pb-0">
		<div class="container">
			@include('helpers.titles.title-2', ['title' => trans('global.countries')])
			...
			<div class="row{{ $mb }}">
				<div class="col-md-12">
					<div class="container bg-body-tertiary rounded py-3">
						...
						<h3 class="fs-3 mb-4">
							<i class="bi bi-geo-alt"></i> {{ trans('global.select_a_country') }}
						</h3>
						...
						@if ($countries->isNotEmpty())
							<div class="row row-cols-xl-4 row-cols-lg-3 row-cols-2 m-0" id="countryList">
								@foreach ($countries as $code => $country)
									@php
										$countryUrl = dmUrl($country, '/', true, !config('addons.domainmapping.installed'));
										$countryName = $country->get('name');
										$countryNameLimited = str($countryName)->limit(26)->toString();
									@endphp
									<div class="col mb-2">
										@if ($countryFlagShape == 'rectangle')
											<img src="{{ mixStaticFile(url('images/blank.gif')) }}"
						...
						@else
							<div class="row m-0">
								<div class="col-12 text-center mb-3 text-danger fw-bold">
									{{ trans('global.countries_not_found') }}
								</div>
							</div>
						@endif
